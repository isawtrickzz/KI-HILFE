...
{
    float chance = victim->GetTotalAuraModifier(SPELL_AURA_MOD_DODGE_PERCENT);
    
    // Neue Ausweichberechnung mit Rüstungsbegrenzung
    float armorValue = victim->GetArmor();
    float armorCap = 12000.0f; // Maximale effektive Rüstung
    
    if (armorValue > armorCap)
        armorValue = armorCap;
    
    chance += (armorValue / armorCap) * 10.0f; // Maximaler Bonus von 10% durch Rüstung
    
    // Absolute Obergrenze für Ausweichen
    if (chance > 25.0f)
        chance = 25.0f;
